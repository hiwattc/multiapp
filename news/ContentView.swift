...
class ArtService {
    ...
    func searchArtworks(query: String = "painting") async throws -> [Int] {
        ...
        return Array((response.objectIDs ?? []).prefix(50)) // 최대 50개
    }
    ...
    func getFeaturedArtworks() async throws -> [Artwork] {
        ...
        let featuredIDs = [
            ...
            438754, // Gauguin - Ia Orana Maria
            436528  // Monet - Garden at Sainte-Adresse
        ]
        ...
        for id in featuredIDs {
            do {
                let artwork = try await getArtwork(objectID: id)
...
class NewsService {
    ...
    func searchNews(query: String) async throws -> [Article] {
        ...
        let urlString = "\(baseURL)/everything?q=\(query)&from=\(fromDate)&to=\(today)&sortBy=publishedAt&apiKey=\(apiKey)"
        guard let encodedURL = urlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let url = URL(string: encodedURL) else {
            throw URLError(.badURL)
        }
...
struct MapView: View {
    ...
    @State private var selectedMapType: MapType = .openStreetMap
                        ...
                        Button(action: {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                                if let location = locationManager.location {
                                    executeMapScript("map.setView([\(location.latitude), \(location.longitude)], 15);")
                                }
                            }
                        }) {
